...
(function () {
  ...
  function cardHTML(r) {
    ...
    return (
      '<div class="recipe-card" data-id="' + r.id + '" tabindex="0" role="button" aria-label="' + escapeHTML(r.name) + '">' +
  ...
  function hideSplashFallback() {
    // The splash normally hides itself via the CSS animation in style.css.
    // If that stylesheet is slow to arrive (e.g. very first load, before the
    // service worker has anything cached) the splash could otherwise be
    // stuck on screen forever with the CSS still pending. This is a
    // JS-only safety net that force-hides it a moment after load.
